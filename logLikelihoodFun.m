...
function [ll, grad] = logLikelihoodFun(w_opt)
    ...
    try
        ...
        beta  = 1.0;  % lesser priority on spread
        gamma = 1.0;
    ...
    %     activeIdxLocal = validatedIdx( ...
    % (abs(log2(y4h_norm(validatedIdx))) > threshold) | (abs(log2(ylong_4h(validatedIdx))) > threshold) );
        [accuracy, ~, ~, ~, ~, ~, ~, ~] = ...
    EvaluateGenePrediction(log2(y4h_norm), log2(ylong_4h), activeIdxLocal, threshold);
